...
DEFCONFIG="${1:-arch/arm64/configs/gki_defconfig}"

if [[ ! -f "$DEFCONFIG" ]]; then
  echo "Error: Defconfig file $DEFCONFIG not found!"
  exit 1
fi

echo "⚙️ Added KSU & SuSFS configuration to $DEFCONFIG"
...
EOF
  ...
  echo "⚙️ LTO Optimization skipped (For KVER ${KVER:-unknown})"
fi
