...
# Constants
WORKDIR="$(pwd)"
KVER="${KVER:-6.6}" # Default to 6.6 if not set
...
if [[ "${KVER}" == "6.6" ]]; then
  ...
  KERNEL_REPO="https://github.com/DikyVinus/android_kernel_common_android12-5.10.git"
  ANYKERNEL_BRANCH="master"
  KERNEL_BRANCH="mahiru-snowdrop-release"
fi
...
export KSU
export KSU_SUSFS
# Use local script instead of downloading from GitHub
bash "${WORKDIR}/inject_ksu/gki_defconfig.sh" "${DEFCONFIG_FILE}"
# --------------------------------------
...
case "${KSU:-}" in
  "yes") VARIANT="KSU" ;;
  "Supported Unofficial Manager") VARIANT="ReSukiSU" ;;
  "no") VARIANT="VNL" ;;
  *) VARIANT="UNK" ;;
esac
...
if [[ -z "${CLANG_BRANCH}" ]]; then
  ...
  curl -fsSL "$CLANG_URL" -o clang-archive
  mkdir -p "$CLANG_DIR"
  case "$(basename "$CLANG_URL")" in
    ...
    *.7z)
      ...
      ;;
    *)
      error "Unsupported Clang file format"
      ;;
  esac
  rm clang-archive
...
# --- ReSukiSU Setup Block (Separated Logic) ---
  ...
  if [[ "${KVER}" == "5.10" ]]; then
    ...
    rm -rf sus/.git
    susfs_patches_dir=sus/kernel_patches
    cp -r "$susfs_patches_dir/fs" .
    cp -r "$susfs_patches_dir/include" .
    cp -r "$susfs_patches_dir/50_add_susfs_in_${SUSFS_BRANCH}.patch" .
...
if susfs_included; then
  if [[ "${KSU:-}" != "resukisu" ]] || ([[ "${KSU:-}" == "resukisu" ]] && ([[ "${KVER}" == "6.1" ]] || [[ "${KVER}" == "6.6" ]])); then
    ...
    SUSFS_PATCHES_DIR="${SUSFS_DIR}/kernel_patches"
    ...
    cp -R "$SUSFS_PATCHES_DIR/fs"/* ./fs
    cp -R "$SUSFS_PATCHES_DIR/include"/* ./include
    patch -p1 < "$SUSFS_PATCHES_DIR/50_add_susfs_in_${SUSFS_BRANCH}.patch" || true

    FIRST_4_VCODE=$(echo "$LINUX_VERSION_CODE" | head -c4)
    FIRST_2_VCODE=$(echo "$LINUX_VERSION_CODE" | head -c2)
    FIRST_3_VCODE=$(echo "$LINUX_VERSION_CODE" | head -c3)
    FIRST_1_VCODE=$(echo "$LINUX_VERSION_CODE" | head -c1)

    if [[ "$FIRST_4_VCODE" == "6630" ]]; then
      patch -p1 < "$KERNEL_PATCHES/susfs/namespace.c_fix.patch"
      patch -p1 < "$KERNEL_PATCHES/susfs/task_mmu.c_fix.patch"
    elif [[ "$FIRST_4_VCODE" == "6658" ]]; then
      patch -p1 < "$KERNEL_PATCHES/susfs/task_mmu.c_fix-k6.6.58.patch"
    elif [[ "$FIRST_2_VCODE" == "61" ]]; then
      patch -p1 < "$KERNEL_PATCHES/susfs/fs_proc_base.c-fix-k6.1.patch"
    elif [[ "$FIRST_3_VCODE" == "510" ]]; then
      patch -p1 < "$KERNEL_PATCHES/susfs/pershoot-susfs-k5.10.patch"
    fi

    if [[ "$FIRST_1_VCODE" == "6" ]]; then
...
if [[ -n "${DEFCONFIG_TO_MERGE:-}" ]]; then
  log "Merging configs..."
  if [[ -f "scripts/kconfig/merge_config.sh" ]]; then
    for cfg in $DEFCONFIG_TO_MERGE; do
      make "${MAKE_ARGS[@]}" scripts/kconfig/merge_config.sh "$cfg"
    done
  else
...
if [[ -f "$KERNEL_IMAGE" ]]; then
  ...
  curl -fsSL "https://github.com/Kingfinik98/SukiSU_patch/raw/refs/heads/main/kpm/patch_linux" -o patch
  chmod +x patch
  ./patch
  if [[ -f oImage ]]; then
    mv -f oImage Image
  ...
  log "Warning: Image file not found in $KERNEL_IMAGE. Skipping KPM patch."
fi
# ----------------------------------------------------
...
if [[ "${LAST_BUILD:-}" == "true" ]]; then
  {
    ...
    echo "SUSFS_VERSION=${SUSFS_VERSION:-none}"
